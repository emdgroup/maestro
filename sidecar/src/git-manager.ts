...
export async function createWorktree(
  ...
  try {
    ...
    throw new Error(
      `Failed to create worktree ${worktreeId}: ${
...
export async function deleteWorktree(
  ...
  try {
    console.log(
      `Deleting worktree: ${worktreeId} (branch: ${branchName})`
    );
    ...
    try {
      ...
      throw new Error(
        `Failed to remove worktree ${worktreeId}: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
    }
...
export async function resetWorktree(
  ...
  try {
    ...
    throw new Error(
      `Failed to reset worktree ${worktreeId}: ${
        error instanceof Error ? error.message : String(error)
      }`
    );
  }
