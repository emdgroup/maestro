 ...
 */
export async function getDiffBetweenBranches(repoPath, fromBranch, toBranch, contextLines = 6) {
...
async function main() {
    ...
    if (args.includes("--get-diff")) {
        ...
        if (!repoPath || !fromBranch || !toBranch) {
            console.error("Usage: node index.js --get-diff <repoPath> <fromBranch> <toBranch> [contextLines]");
            process.exit(1);
        }
        try {
