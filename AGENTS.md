...
SQLite with foreign key constraints enabled. Schema V26. Configured with WAL mode and 5s `busy_timeout` for concurrent access.
...
| ------------------- | -------------------------------------------------------------------------- |
| `0` (fresh install) | create the full schema from `SCHEMA_V26_FULL`                              |
...
- Schema version: 26 (`SCHEMA_VERSION` in `core/schema.rs`). Databases at v22 or later migrate in place and keep their data; only pre-v22 databases are dropped and recreated
