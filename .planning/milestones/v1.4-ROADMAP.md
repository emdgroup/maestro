# Milestone v1.4: Quality & Worktrees
...
- [x] 40-03-PLAN.md — Frontend: useConnectionHealth hook + DisconnectBackdrop component + App.tsx wiring
...
**Key Accomplishments:**
- Removed Node.js sidecar entirely; squash merge now native Rust subprocess
- Worktrees view redesigned: card grid grouped by origin branch, slide-in diff panel, schema V6 (base_branch + AheadBehind)
- In-diff git commit workflow: file-level tri-state checkboxes, hunk-level staging, revert/shelve actions
- Fixed SSH terminal "cached screen" bug: history as trimmed String, live vs dead session split, DB persistence on exit
- SSH disconnection handling: russh keepalive, heartbeat polling, full-screen reconnect backdrop with exponential backoff
- ACP infrastructure: Cargo workspace root, maestro-protocol wire types, MaestroAcpClient stub, maestro-server binary skeleton
- Full backend code quality sweep: 30 findings fixed (panics, DRY violations, shell injection, password zeroing, NULL semantics)
- Unified `run_git_in_dir` dispatcher for local+remote git ops; git2 dependency removed

**Tech Debt Incurred:**
- MaestroAcpClient stubs return `Err(method_not_found())` — Phase 42 wires real ACP protocol
- maestro-server `main.rs` has compile-time import placeholder — Phase 42 replaces with real stdin reader
