...
function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  return (
    <InputPrimitive
      ...
      data-slot="input"
      className={cn(
        // `border-border`, not stock shadcn's `border-input`: in this theme --input is a fill
        // colour, not an edge. It is oklch(99%) in light and oklch(32% 0.01 250) in dark — the
        // latter byte-identical to --card and --popover, so a border-input edge inside any dialog
        // is invisible. The `dark:bg-input/30` fill below is what --input is actually for.
