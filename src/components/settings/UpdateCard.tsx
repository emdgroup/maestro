...
export function UpdateCard() {
  const { status, lastChecked, checkForUpdates, install, applyUpdate } = useUpdater();
  ...
  // Show restart dialog automatically when download finishes
  useEffect(() => {
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-muted-foreground mt-0.5">
              {status.phase === "available" ? (
                ...
                <>
                  v{appVersion} <span className="text-muted-foreground">→</span>{" "}
                  <span className="text-amber-500 font-medium">v{status.version}</span>
                </>
              ) : (
                <>v{appVersion}</>
              )}
        ...
        <DialogContent showCloseButton={false} className="max-w-sm">
          <DialogHeader>
            <div className="flex items-center gap-3 mb-1">
              <img src={appIconUrl} alt="Maestro" className="w-9 h-9 rounded-lg shrink-0" />
              <DialogTitle className="text-base">Update ready to install</DialogTitle>
            </div>
            <DialogDescription>
              Maestro{newVersion ? ` v${newVersion}` : ""} has been downloaded and is ready. Restart
              the app to finish the update.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <button
