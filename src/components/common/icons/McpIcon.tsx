...
/**
 * The Model Context Protocol mark, from the spec's own brand assets.
 *
 * A component rather than an `.svg` file because the mark has to take the colour
 * of whatever it sits in — a tool row tints its icon by status. There is no
 * `vite-plugin-svgr` here, so an imported `.svg` is only ever a URL, and an
 * `<img>` cannot inherit `currentColor`.
 *
 * Takes plain `SVGProps`, the same shape as the `@thesvg/react` icons used
 * elsewhere, so it drops into the same slots. Size it with a class.
 */
