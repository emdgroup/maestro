 ...
 *
 * A placeholder that looks like a value is read as one, and a token like `<id>-task` is read as a
 * form to fill in — it reproduces the shape of the answer and brackets the part to replace, which
 * is exactly how a template looks. Prose cannot be mistaken for either, and "by default" is the
 * only thing that says the field may be left alone.
 */
const UNKNOWN_NAME_PLACEHOLDER = "Auto-generated by default";
...
export interface NewWorktreeFieldsProps {
  ...
  /**
   * What the name will be if the field is left alone, shown as its placeholder — pass it only when
   * it is an actual branch name, so the greyed text can be trusted as one.
   *
   * Null when the name cannot be known at render: a task's carries an id the task does not have
   * yet, and an unnamed session's is generated at spawn. The placeholder then says the name is
   * generated instead of miming a value nothing will use.
   */
  generatedSuffix: string | null;
        ...
        <div className="flex flex-col gap-1">
          <InputGroup className={cn(suffixError && "border-destructive")}>
            ...
            <InputGroupInput
              ...
              // The placeholder is a real branch name only when `generatedSuffix` is one; the hint
              // is prose, so it drops the monospace the field otherwise types in.
              className={cn("font-mono text-xs", !generatedSuffix && "placeholder:font-sans")}
              placeholder={generatedSuffix ?? UNKNOWN_NAME_PLACEHOLDER}
              value={branchSuffix}
