...
pub fn list_containers(cli: &ContainerCli) -> Result<Vec<DockerContainer>, String> {
    ...
    Ok(parse_container_list(&String::from_utf8_lossy(
        &output.stdout,
    )))
}
...
pub(crate) async fn run(
    ...
    crate::connectivity::exec_channel::run(
        &crate::connectivity::exec_channel::ExecTarget::Docker {
            cli: *cli,
...
pub async fn copy_from(
    ...
    if let Some(parent) = host_path.parent() {
        tokio::fs::create_dir_all(parent)
            .await
            .map_err(|e| format!("Cannot create local directory '{}': {e}", parent.display()))?;
    }
    let dest = host_path
        .to_str()
