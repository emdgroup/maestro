...
pub async fn save_project_issue_tracking_config(
    ...
    mutate_project_config(&conn, |config| {
        ...
        config.issue_tracking_auto_detect = if issue_tracking.is_none() {
            Some(false)
        } else {
            None
        };
...
pub(crate) fn provider_for_host(host: &str, app_state: &AppState) -> Option<String> {
    ...
    stored_integrations(app_state)
        .into_iter()
        .find(|(_, creds)| {
            creds.instance_url.as_deref().and_then(url_host).as_deref() == Some(host)
        })
...
pub(crate) async fn find_integration(
    ...
    match provider {
        "github" => crate::integration::github::try_gh_cli_token()
            .await
            .map(|token| MatchedIntegration {
                id: "gh_cli".to_string(),
                token,
                instance_url: None,
            }),
        "gitlab" => crate::integration::gitlab::try_glab_cli_credentials()
            .await
            .map(|(token, instance_url, _display_name)| MatchedIntegration {
        ...
        if provider == "gitlab" {
            if let Some(project_path) = &config.project_path {
                ...
                .await
                {
                    ...
                    Err(e) => log::warn!(
                        "Issue tracking detection: GitLab project lookup failed: {}",
                        e
                    ),
                }
            }
    ...
    match provider.as_str() {
        ...
        "gitlab" => {
            let (token, instance_url) =
                match get_integration_creds_for_project("gitlab", &ticketing, &app_state) {
                    Ok(creds) => {
                        let url = creds.instance_url.ok_or_else(|| {
                            "GitLab: instance_url missing from stored credentials".to_string()
                        })?;
                        (creds.token, url)
                    }
                    Err(_) => {
                        let (cli_token, cli_url, _) =
                            crate::integration::gitlab::try_glab_cli_credentials()
                                .await
            ...
            let gitlab_project_id: i64 = ticketing
                ...
                .as_deref()
                .ok_or_else(|| {
                    "GitLab: project_key (numeric id) required in project ticketing config"
                        .to_string()
                })?
                .parse()
        ...
        "forgejo" => {
            ...
            let instance_url = creds.instance_url.as_deref().ok_or_else(|| {
                "Forgejo: instance_url missing from stored credentials".to_string()
            })?;
            let owner = ticketing
                .owner
                .as_deref()
        ...
        "linear" => {
            ...
            crate::integration::linear::fetch_issues(&creds.token, ticketing.team_id.as_deref())
                .await
        }

        "jira_cloud" => {
            let creds = get_integration_creds_for_project("jira_cloud", &ticketing, &app_state)?;
            let site_url = creds.instance_url.as_deref().ok_or_else(|| {
                "Jira Cloud: site_url missing from stored credentials".to_string()
            })?;
            let email = creds
                .email
                .as_deref()
                .ok_or_else(|| "Jira Cloud: email missing from stored credentials".to_string())?;
            let project_key = ticketing.project_key.as_deref().ok_or_else(|| {
                "Jira Cloud: project_key required in project ticketing config".to_string()
            })?;
            crate::integration::jira_cloud::fetch_issues(site_url, email, &creds.token, project_key)
                .await
        }
        ...
        "azuredevops" => {
            ...
            let org_url = creds.instance_url.as_deref().ok_or_else(|| {
                "Azure DevOps: org_url missing from stored credentials".to_string()
            })?;
            let project_name = ticketing.project_name.as_deref().ok_or_else(|| {
                "Azure DevOps: project_name required in project ticketing config".to_string()
            })?;
            crate::integration::azure_devops::fetch_issues(org_url, project_name, &creds.token)
                .await
        }
